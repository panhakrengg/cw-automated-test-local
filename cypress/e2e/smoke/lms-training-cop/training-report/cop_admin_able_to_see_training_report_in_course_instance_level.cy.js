...
describe(Epic.LmsTrainingCop, () => {
  ...
  before(() => {
    ...
    trainingReportYamlStub.getCopCourseYaml((courseData) => {
      courseSource = courseData.tennisByYourself
      courseInstance = courseSource.courseInstances.bookedOptionalActivity
      copManageCourse.setCourse(courseSource)
    })
  ...
  context(Story.trainingReport, () => {
    it('CoP Admin able to see training report in course instance level', () => {
      Story.ticket('QA-935')
      ...
      context('Search course target and click on three dot Training Reports option', () => {
        ...
        copManageCourse.clickOnCourseInstanceThreeDotOption(courseInstance.date, 'Training Reports')
      })
